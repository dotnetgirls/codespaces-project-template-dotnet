...
이 포트폴리오 사이트에는 세 가지 이미지가 있습니다 : 상단 배경, "About me" 배경, 포트폴리오 섹션(책상 그림). 이 기본 세 가지 이미지들 대신 사용 허가 없이 자유롭게 사용할 수 있는 이미지나 자신이 소유하고 있는 이미지 중 **가로 형태**의 이미지를 넣으면 됩니다.
...
`/src/BlazorApp/wwwroot/sample-data/projects.json` 파일 내 JSON 배열의 강조하려는 각 항목에는 **제목, 설명, URL** 이 필요합니다.

예시 내용에는 네 개의 항목이 있지만, 항목 수는 자유롭게 선택하면 됩니다.
...
[Azure 정적 웹앱](https://azure.microsoft.com/products/app-service/static/?WT.mc_id=dotnet-82024-juyoo)은 Microsoft의 정적 사이트(또는 서버가 아닌 사용자의 브라우저에서 렌더링 되는 사이트)를 위한 호스팅 솔루션입니다. 이 서비스는 Azure Functions, 인증, 스테이징 버전 등 Azure를 통해 사이트를 확장할 수 있습니다.
